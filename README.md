...
Data was inserted using **manual INSERT statements** written directly into `init-db/insertTables.sql`.

The file contains INSERT statements for all 14 tables:
- All 8 lookup tables are seeded with real, meaningful values (e.g. `'active'`, `'suspended'`, `'banned'` for player statuses).
- All 6 core tables are populated with at least 500 records each.
...
A binary dump was created using the `pg_dump` utility from inside the Docker container:

```bash
pg_dump -U admin_chess -Fc chess_db > backups/backup_2026_04_28.dump
```

Backup file: [`backups/backup_2026_04_28.dump`](backups/backup_2026_04_28.dump)

To restore on another machine:

```bash
pg_restore -U admin_chess -d chess_db backups/backup_2026_04_28.dump
```
...
2. Set format to **Custom**, filename to `backup_ui.dump`, and click **Backup**.

Backup file: [`backups/backup_ui.dump`](backups/backup_ui.dump)
