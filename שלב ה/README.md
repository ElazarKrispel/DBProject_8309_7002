...
- **מפתחות זרים כשמות** - בטבלאות ובטפסים מוצגים שמות משמעותיים במקום מזהי FK. מזהים נשמרים בתוצאות SQL ובתוצאות תוכניות לצורך בדיקה, ובטבלאות ניתן לראותם ברמז בעת ריחוף.
...
| Node.js + npm | 20.19 ומעלה בסדרת 20, או 22.12 ומעלה | דרישת Vite 8; פותח עם Node 24 |
...
מחיקת volume או שחזור ידני מוחקים את המצב הקיים. אם נוצרו שינויים שרוצים לשמור, יש לגבות אותם לפני שחזור. ההפעלה הרגילה אינה מחייבת שחזור חוזר.
...
הסקריפט מתקין תלויות חסרות ובונה את ה-frontend בכל הפעלה (`npm run build`), מתקין את תלויות ה-backend (`pip install -r requirements.txt`), בודק את קוד היציאה של כל שלב ועוצר במקרה של כישלון. לאחר הצלחה הוא מרים את השרת ללא `--reload` על `http://localhost:8000` ופותח את הדפדפן.
...
| Dashboard מציג "Stage 4 programs need attention" | volume ישן בלי הרוטינות של שלב ד. לבצע שחזור לפי שלב 1 |
...
| שכבה | כלי | למה |
|---|---|---|
| Frontend | React 19.2.8 + TypeScript 6 + Vite 8 | מודל קומפוננטות, טיפוסים שנגזרים מחוזה ה-API, שרת פיתוח מהיר עם proxy |
| רכיבי UI | Mantine 8 | סט שלם: טבלאות, מודלים, שדות תאריך, התראות, Spotlight, גרפים, מצב כהה מובנה. חסך לנו כתיבת רכיבים בסיסיים |
...
1. **שחזור הבסיס ואימות.** התחלנו משחזור `backup4.sql` לתוך הקונטיינר ואימות שכל אובייקטי שלב ד קיימים (27 טבלאות, 3 מבטים, 4 רוטינות, 2 טריגרים). ה-Dashboard מציג את מצב ההתקנה בתג Stage 4 programs installed.
2. **חוזה API קודם לקוד.** כתבנו תחילה את חוזה ה-API (הנתיבים, הפרמטרים וצורת התשובה של כל endpoint), ורק אז פיתחנו את ה-backend ואת ה-frontend במקביל מולו. הטיפוסים ב-`src/api/types.ts` ובמסכים מתארים את חוזה ה-API בפועל.
...
- כל 30 מסכי הטבלאות והמבטים נפתחו בדפדפן. ב-27 הטבלאות נפתחו טופסי הכנסה ועריכה; שלושת המבטים הוצגו לקריאה בלבד.
- CRUD מלא נבדק דרך הממשק בטבלאות `club`, `player_status` ו-`engine_ui_support`; עדכון שחקן לפי מפתח נבדק עם החזרת הערך המקורי. רשומות הבדיקה נמחקו.

- `python -m app.smoke_core`: 68 בדיקות לכניסה, מטא-נתונים, CRUD ומסוף SQL.
- `python -m app.smoke_programs`: 126 בדיקות לקטלוג שאילתות שלב ב ולתוכניות שלב ד.
- במסוף SQL נבדקו שמונה הדוגמאות, שגיאת עמודה לא קיימת ועדכון בתצוגה מקדימה; קריאה חוזרת אימתה שהעדכון לא נשמר.
- פרטי שחקן, שחקן חסר, פרטי מועדון, לשוניות הפעילות ושתי הדגמות הטריגרים נבדקו בדפדפן מול הבסיס החי.
- שבעת המסכים הראשיים נבדקו ברוחבי 390, 768 ו-1440 פיקסלים ללא חריגת רוחב העמוד, וכן נבדק מצב כהה.

### שתי החלטות תכנון שכדאי להכיר

- **מזהים.** בסכמה אין sequences; המפתחות המספריים מוגדרים ללא ברירת מחדל אוטומטית. לכן בהכנסה של רשומה חדשה השרת מחשב `COALESCE(MAX(pk), 0) + 1` באותה טרנזקציה של ה-INSERT. בטבלאות עם מפתח מורכב (`servercomponent`, `engine_ui_support`, `installed_on`) או מפתח מחרוזתי (טבלאות ה-lookup) המשתמש מזין את המפתח בעצמו. בטבלאות `localengine` ו-`cloudengine`, שמפתחן הוא גם FK, בוחרים את המנוע הקיים.
- **שורות ריפוד ב-lookup.** כל טבלת lookup מכילה 500 שורות, אבל רק 2-5 מהן קודי הפעילות שבשימוש (`active`, `banned`, `monthly` וכו'); השאר שורות ריפוד (`ps0007` וכדומה). בוחרי ה-FK מסננים את הריפוד ומציגים את הקודים האמיתיים תחילה, ורק חיפוש מפורש מגיע לשורות הריפוד. טבלת `subscription_tier` אינה מסוננת כך: כל 500 התוכניות משמשות מנויים ומוצגות עם חיפוש.
...
- שגיאות PostgreSQL מוצגות עם ההודעה וקוד השגיאה. CRUD מחזיר HTTP 400 כשאילוץ נכשל; מסוף SQL והדגמות התוכניות מחזירים גוף תוצאה עם `ok: false` ופרטי שגיאה גם כאשר סטטוס HTTP הוא 200.
- הכניסה מבוססת bearer token שנשמר בזיכרון השרת; נתיבי הנתונים דורשים אותו; הכניסה ובדיקת חיות השרת זמינות ללא token.
...
| GET | `/api/tables/{t}/row?player_id=..` | שורה בודדת לפי מפתח (הבסיס ל"עדכון לפי מפתח") |
| POST | `/api/tables/{t}` | הכנסה; מפתח מספרי אוטומטי מחושב `MAX+1`, למעט PK שהוא גם FK |
| PUT | `/api/tables/{t}?player_id=..` | עדכון חלקי |
| DELETE | `/api/tables/{t}?player_id=..` | מחיקה |
...
בנתיבי רשומה משתמשים בשם המפתח של הטבלה בפועל; `player_id` בדוגמה מתאים ל-`player`. במפתח מורכב מעבירים את כל עמודות המפתח.
...
הכניסה למערכת. שם משתמש וסיסמה נבדקים מול `APP_USER` / `APP_PASSWORD`, ובהצלחה מתקבל token שמצורף לכל בקשה. לאחר הכניסה מגיעים ל-Dashboard, ומשם דרך סרגל הניווט לכל מסך אחר במערכת.

![מסך כניסה](screenshots/01_login.png)

### 5.2 Dashboard

מסך הבית אחרי הכניסה, עם הכותרת **Overview**. מציג שמונה כרטיסי KPI (שחקנים עם מספר הפעילים בתיאור, מועדונים, חברויות פעילות, מנויים פעילים, התחברויות, אחוז התחברויות חשודות, מנועים וצומתי חומרה), גרפים (התחברויות לפי חודש, התפלגות דירוגים, תוכניות המנוי הפופולריות, שחקנים לפי מדינה וסטטוס וחברויות לפי תפקיד), כרטיסי ניווט מהיר לכל מודול, ותג **Stage 4 programs installed** שמאמת שכל הפונקציות, הפרוצדורות והטריגרים מותקנים. הנתונים נשלפים ב-`GET /api/dashboard` מהטבלאות `player`, `club`, `club_membership`, `player_subscription`, `login_log`, `engine`, `hardwarenode`.

![Dashboard](screenshots/02_dashboard.png)

### 5.3 מסכי הטבלאות (27 טבלאות + 3 מבטים)

מסך אחד גנרי, `/tables/:key`, משרת את כל הישויות. בסרגל הניווט הן מקובצות: **Players & Clubs** (`player`, `club`, `club_membership`, `social_connection`), **Billing** (`player_subscription`, `subscription_tier`), **Security** (`login_log`), **Reference Data** (8 טבלאות ה-lookup), **Engines & Infrastructure** (12 טבלאות האגף השותף) ו-**Views** (3 המבטים, לקריאה בלבד).

מה אפשר לעשות בכל טבלה:

- **שליפה** - חיפוש טקסט חופשי בעמודות הרלוונטיות, מיון לפי כל עמודה, עימוד, וסינון לפי ערכי FK ו-lookup.
- **מפתחות זרים כשמות** - `status_code` מוצג כ-"Active" ולא `active`, `player_id` מוצג כשם המשתמש, `club_id` כשם המועדון, `engine_id` כשם המנוע וגרסתו. המזהה עצמו נשאר זמין ברמז בעת ריחוף.
- **הכנסה** - כפתור New record פותח טופס שנבנה מהמטא-נתונים: בוחר FK עם חיפוש, בוחר תאריך, מתג לבוליאני, שדה מספרי, שדה מוסתר לעמודות סודיות (`cloudengine.auth_token`). מפתח מספרי רגיל מחושב אוטומטית; מפתח מחרוזתי, מורכב או PK שהוא גם FK מוזן או נבחר בטופס.
...
הטבלאות של האגף השותף מוצגות באותו מנוע. בצילום מוצגת `engine_ui_support`, עם מפתח מורכב ושמות המנוע והלקוח במקום מזהי המפתחות הזרים:

![טבלאות האגף השותף](screenshots/18_partner_tables.png)

### 5.4 עדכון לפי מפתח

הדרישה המפורשת של שלב ה: המשתמש ממלא את המפתח, המערכת מביאה את יתר השדות, ומשם מעדכנים. בכל מסך טבלה יש פאנל **Update by key**: מקלידים את ערך המפתח (או כמה ערכים בטבלה עם מפתח מורכב), לוחצים Load record, השרת מחזיר את השורה מ-`GET /api/tables/{t}/row`, הטופס מתמלא, המשתמש משנה מה שצריך ולוחץ Save record. אם המפתח לא קיים מוצגת הודעה ברורה.
...
`/clubs/:id`. כותרת המועדון (מדינה, עיר, רשמי/לא רשמי, תאריך הקמה), טבלת החברים הפעילים מהמבט `vw_player_club_membership`, עם קישור לשחקן, שם מלא, מדינה, דירוג, תפקיד ותאריך הצטרפות. חיפוש מקומי מסנן את הטבלה. מוצגים חברים פעילים, מספר בעלים ומנהלים, דירוג קלאסי ממוצע, תאריך ההצטרפות הוותיק וגרף התפלגות תפקידים.

![Club detail](screenshots/08_club_detail.png)

### 5.7 Reports: שאילתות שלב ב

`/reports`. קטלוג של 14 כרטיסים: Q1-Q8 (SELECT), U1-U3 (UPDATE), D1-D3 (DELETE). לכל שאילתה טופס פרמטרים (למשל `min_clubs` ב-Q5, טווח שנים ב-Q2, מספר ימים ב-D1), הצגת ה-SQL, טבלת תוצאות עם זמן ריצה, וגרף היכן שזה טבעי (Q2 קו לפי חודש, Q3 עמודות לפי תוכנית). ב-Q5-Q8, שנכתבו בשלב ב בשתי גרסאות, מתג **A/B** מחליף בין הגרסאות ומציג את ה-SQL של הגרסה הנבחרת. ההסבר נמצא תחת Why is one form faster?:
...
`/programs`. לשוניות Functions, Procedures ו-Triggers. שתי הפונקציות ושתי הפרוצדורות מוצגות בכרטיסים עם טופס פרמטרים שברירות המחדל שלו יחסיות לנתונים (ההתחברות האחרונה בבסיס היא 2026-03-24):
...
**הדגמות הטריגרים.** לשונית Triggers במסך שמדגים את תרחישי הטריגרים של שלב ד, ומציג את ההודעה של הבסיס ואת הראיות לפני/אחרי:
...
`/sql`. עורך SQL חופשי עם שמונה דוגמאות מוכנות, קיצור Ctrl+Enter, היסטוריית עשר הפקודות האחרונות בדפדפן וייצוא התוצאה ל-CSV. התוצאה מציגה עמודות, מספר שורות וזמן ריצה; הודעות `RAISE NOTICE` ושגיאות מוצגות מתחתיה. במצב Preview כל הטרנזקציה מסתיימת ב-ROLLBACK, גם עבור SELECT ואצווה של מספר פקודות. Apply דורש אישור ושומר ב-COMMIT. פקודות ניהול טרנזקציה מפורשות, כגון COMMIT ו-ROLLBACK, נחסמות: השליטה נעשית דרך Preview / Apply.

![SQL Console](screenshots/17_sql_console.png)

### 5.10 מעטפת המסכים

סרגל ניווט מקובץ עם אייקון לכל טבלה, פירורי לחם, התראות אחרי שמירת ומחיקת רשומות, ותוצאות מפורשות אחרי הרצת שאילתות ותוכניות, מצבי טעינה/ריק/שגיאה אחידים, תפריט משתמש עם יציאה. מתג מצב כהה בכותרת, ו-**Spotlight** (`Ctrl+K`) לחיפוש מהיר של כל טבלה ומסך לפי שם.
...
| הכנסה לכל הטבלאות | New record; מפתח לפי סוגו; בוחרי FK | 04 |
...
| הכנסה לכל הטבלאות | `POST /api/tables/{t}`, טיפול במפתח מספרי, מחרוזתי, מורכב או PK-FK | ✔ |
...
| שמות משמעותיים במקום מזהי FK בטבלאות ובטפסים | צירופים בצד השרת (`_labels`), בוחרי FK עם שמות | ✔ |
...
- **מנוע אחד ל-30 מסכים.** מסכי הטבלאות לא נכתבו אחד-אחד; הם נגזרים מהסכמה ומרשימת הרישום. פרטי התצוגה מוגדרים ב-`registry.py`, והעמודות והאילוצים נקראים מקטלוג הבסיס.
...
- **Preview לפני Apply.** ברירת המחדל להרצת DML של שלב ב, הפרוצדורות, הדגמות הטריגרים וקונסולת SQL היא טרנזקציה שמסתיימת ב-`ROLLBACK`, כך שאפשר להדגים שוב ושוב על אותו baseline, בדיוק כמו בשיטת ההוכחה של שלב ד.
